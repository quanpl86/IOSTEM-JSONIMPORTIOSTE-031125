...
def main():
    ...
    for index, row in df.iterrows():
        ...
        map_request = {
            ...
            "translations": {}, # Khởi tạo translations rỗng
            "generation_config": {
        ...
        # [SỬA LỖI] Làm việc trực tiếp với dictionary `translations` của `map_request`
        # thay vì tạo một biến mới.
        translations = map_request['translations']
        if 'title_vi' in row and row['title_vi']:
            if 'vi' not in translations: translations['vi'] = {}
            translations['vi'][f"Challenge.{row['id']}.Title"] = row['title_vi']
            translations['vi'][f"Challenge.{row['id']}.Description"] = row['description_vi']
        if 'title_en' in row and row['title_en']:
            if 'en' not in translations: translations['en'] = {}
            translations['en'][f"Challenge.{row['id']}.Title"] = row['title_en']
            translations['en'][f"Challenge.{row['id']}.Description"] = row['description_en']
